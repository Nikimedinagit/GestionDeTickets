...
export function CategoriasFormModal({
  isOpen,
  onClose,
  onSave,
  ...
  //** useEffect para cargar los datos de las categorias a editar cuando se abre el modal, o limpiar el formulario si no hay categorias o se cierra el modal. */
  useEffect(() => {
    if (categoriaAEditar && isOpen) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSave({
        ...
        estado: categoriaAEditar ? categoriaAEditar.estado : true,
      });
        ...
        <DialogHeader className="p-3">
          <DialogTitle className="text-[#1e293b] flex items-center gap-2 text-xl font-extrabold tracking-tight">
            <div className="bg-[#1e293b]/10 p-2 rounded-lg">
              {categoriaAEditar ? <Pencil size={22} /> : <Layers size={22} />}
            </div>
