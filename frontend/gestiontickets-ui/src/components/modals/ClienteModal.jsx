...
export function ClientesFormModal({
  isOpen,
  onClose,
  onSave,
  clienteAEditar = null,
  ...
  useEffect(() => {
    if (clienteAEditar && isOpen) {
      setFormData({
        nombreCompleto: clienteAEditar.nombreCompleto || "",
        dni: clienteAEditar.dni || "",
        email: clienteAEditar.email || "",
        telefono: clienteAEditar.telefono || "",
        observacion: clienteAEditar.observacion || "",
      });
    } else if (!isOpen) {
      setFormData({
        nombreCompleto: "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSave({
        ...
        estado: clienteAEditar ? clienteAEditar.estado : true,
      });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="w-[calc(100%-2rem)] sm:max-w-[450px] bg-white rounded-md border-none p-0 overflow-hidden shadow-2xl [&>button]:hidden">
        <DialogHeader className="p-3">
          <DialogTitle className="text-[#1e293b] flex items-center gap-2 text-xl font-extrabold tracking-tight">
            <div className="bg-[#1e293b]/10 p-2 rounded-lg">
              {clienteAEditar ? <Pencil size={22} /> : <Users size={22} />}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                value={formData.dni}
                onChange={handleChange}
                disabled={isSubmitting || !!clienteAEditar}
